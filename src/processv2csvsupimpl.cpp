...
namespace pcars
{
    ...
    ProcessV2CSVSupImpl::TimeStamp createTimeStamp()
    {
        time_t rawtime;
        struct tm *timeinfo;
        char buffer[80];
    ...
    void createCSVFile(const ProcessV2CSVSupImpl::TrackName &trackname,
                       const ProcessV2CSVSupImpl::Lap lap,
    ...
    ProcessV2CSVSupImpl::TrackName ProcessV2CSVSupImpl::getTrackName(Packet::Ptr &packet)
    {
        TrackName result;
    ...
    void ProcessV2CSVSupImpl::updateTimingData(Packet::Ptr &packet)
    {
    ...
    void ProcessV2CSVSupImpl::updateNextLap(Packet::Ptr &packet)
    {
        if (packet->type() == PACKETTYPE::PACKETTIMINGDATA)
        {
            PacketTimingData *p = dynamic_cast<PacketTimingData *>(packet.get());
    ...
    void ProcessV2CSVSupImpl::updateRaceState(Packet::Ptr &packet)
    {
        if (packet->type() == PACKETTYPE::PACKETTIMINGDATA)
        {
            PacketTimingData *p = dynamic_cast<PacketTimingData *>(packet.get());
    ...
    void ProcessV2CSVSupImpl::updateTrackName(Packet::Ptr &packet)
    {
        if (!havetrackname_)
        {
    ...
    void ProcessV2CSVSupImpl::updateLapWithCapturedTelemetry()
    {
    ...
    void ProcessV2CSVSupImpl::updateCurrentLap()
    {
        currentlap_ = nextlap_;
    }

    void ProcessV2CSVSupImpl::writeCapturedTelemetryToCSV()
    {
    ...
    bool ProcessV2CSVSupImpl::isFirstOutLapFinshed() const
    {
        return state_ != 9 && !trackname_.empty();
    }

    bool ProcessV2CSVSupImpl::isThisANewLap() const
    {
        return nextlap_ != currentlap_;
    }

    bool ProcessV2CSVSupImpl::isThisTheFirstLap() const
    {
        return currentlap_ == NOTALAP;
    }

    bool ProcessV2CSVSupImpl::isTelemetryEmpty() const
    {
        return data_->telemetry.empty();
    }

    void ProcessV2CSVSupImpl::reset()
    {
